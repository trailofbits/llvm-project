...
/// Return false if inlining would lose the callee's "zeroize-flags" request.
LLVM_ABI bool isZeroizeFlagsInlineCompatible(const Function &Caller,
                                             const Function &Callee);
...
/// not carry the attribute or because the caller's mode clears less of the
/// frame than the callee's does. Return true otherwise.
