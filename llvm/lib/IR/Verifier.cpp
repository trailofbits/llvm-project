...
private:
  ...
  void visitSensitiveMetadata(Instruction &I, MDNode *MD);
...
void Verifier::visitGlobalValue(const GlobalValue &GV) {
  ...
  if (const auto *GO = dyn_cast<GlobalObject>(&GV)) {
    // !sensitive describes a stack object. A global is not one, and nothing
    // clears it on return, so an attachment here would read as a request that
    // is silently never honored.
    Check(!GO->getMetadata(LLVMContext::MD_sensitive),
          "!sensitive metadata can only be applied to alloca instructions", GO);
...
void Verifier::visitSensitiveMetadata(Instruction &I, MDNode *MD) {
  Check(isa<AllocaInst>(I),
        "!sensitive metadata can only be applied to alloca instructions", &I);
  ...
  Check(MD->getNumOperands() == 0, "!sensitive metadata must be empty", &I, MD);
}
...
void Verifier::visitInstruction(Instruction &I) {
  ...
  if (MDNode *MD = I.getMetadata(LLVMContext::MD_sensitive))
    visitSensitiveMetadata(I, MD);
