; !nozeroize marks a stack object, and its presence is the entire signal. Reject
...
; CHECK-DAG: !nozeroize metadata can only be applied to alloca instructions
; CHECK-DAG: ptr @global
@global = global i64 0, !nozeroize !0

; CHECK-DAG: !nozeroize metadata can only be applied to alloca instructions
; CHECK-DAG: ptr @on_function
define void @on_function() !nozeroize !0 {
  ret void
}

; CHECK-DAG: !nozeroize metadata can only be applied to alloca instructions
; CHECK-DAG: store i64 %v, ptr %p, align 8, !nozeroize !0
define void @on_store(ptr %p, i64 %v) {
  store i64 %v, ptr %p, align 8, !nozeroize !0
  ret void
}
...
; CHECK-DAG: !nozeroize metadata can only be applied to alloca instructions
; CHECK-DAG: %v = load i64, ptr %key, align 8, !nozeroize !0
define i64 @on_load_of_marked_object() {
  %key = alloca i64, align 8, !nozeroize !0
  %v = load i64, ptr %key, align 8, !nozeroize !0
  ret i64 %v
}

; CHECK-DAG: !nozeroize metadata must be empty
; CHECK-DAG: %key = alloca i64, align 8, !nozeroize !1
define void @payload_string() {
  %key = alloca i64, align 8, !nozeroize !1
  ret void
}

; CHECK-DAG: !nozeroize metadata must be empty
; CHECK-DAG: %key = alloca i64, align 8, !nozeroize !2
define void @payload_constant() {
  %key = alloca i64, align 8, !nozeroize !2
  ret void
}
